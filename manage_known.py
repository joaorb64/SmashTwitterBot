...
for account in accounts:
    try:
        ...
        for evento in list(events_json):
            print(account+" - checking event: ["+str(evento)+"] "+events_json[evento].get(
                "tournament", "")+" - "+events_json[evento].get("name", ""))
            ...
            try:
                ...
                if data == None:
                    ...
                    if not events_json[evento].get("retries"):
                        ...
                        events_json[evento]["retries"] = int(
                            events_json[evento]["retries"]) + 1
                ...
                if data["state"] == 'COMPLETED' or data["state"] == 'ACTIVE':
                    print("Evento ativo ou completo - " +
                          events_json[evento]["tournament"] + " - " + events_json[evento]["name"])
                    ...
                    for phase in valid_phases:
                        ...
                        for entrant in phase["standings"]["nodes"]:
                            ...
                            for char in char_usage.items():
                                char_in_json = next(
                                    (c for c in characters_json["character"] if c["id"] == char[0]), None)

                                if char_in_json:
                                    char_usage_named[char_in_json["name"]] = {}
                                    char_usage_named[char_in_json["name"]
                                                     ]["name"] = char_in_json["name"]
                                    char_usage_named[char_in_json["name"]
                                                     ]["usage"] = char[1]
                                    char_usage_named[char_in_json["name"]]["icon"] = char_in_json.get("images")[
                                        1].get("url")
                        ...
                        post += "["
                        post += accounts[account]["text-online"] if events_json[evento].get(
                            "isOnline") else accounts[account]["text-offline"]
                        post += "]"
                        post += "\n\n"
                        ...
                        for entrant in phase["standings"]["nodes"]:
                            ...
                            if len(twitter.get("participants")) == 1:
                                if twitter:
                                    twitter = twitter.get("participants", [{}])
                                if twitter:
                                    twitter = twitter[0].get("user", {})
                                if twitter:
                                    twitter = twitter.get(
                                        "authorizations", None)

                            if twitter != None and isinstance(twitter, list) and len(twitter) > 0 and twitter[0] != None and twitter[0].get("externalUsername"):
                                placement_text += "@" + \
                                    str(twitter[0].get("externalUsername"))
                            else:
                                placement_text += entrant["entrant"]["name"]

                                if len(placement_text) > 18:
                                    placement_text = placement_text[:18]+"…"
                            placement_text += "\n"
                        ...
                        if posted:
                            events_json[evento]["postedPhaseResultIds"].append(
                                phase["id"])
                    ...
                    if allPhasesPosted:
                        print("Evento finalizado e postado - " +
                              events_json[evento]["tournament"] + " - " + events_json[evento]["name"])
                        events_json.pop(evento)
                        continue

                # Evento iniciado
                if not "postedStarting" in events_json[evento] and data["state"] == 'ACTIVE':
                ...
                if not "postedRegistrationClosing" in events_json[evento].keys() and "tweet_id" in events_json[evento].keys():
                    if events_json[evento]["state"] != 'ACTIVE' and min(events_json[evento]["tournament_registrationClosesAt"], events_json[evento]["startAt"]) <= time.time() + datetime.timedelta(hours=1).total_seconds():
                        print("Menos de 1h para fechar inscrições - " +
                              events_json[evento]["tournament"] + " - " + events_json[evento]["name"])
